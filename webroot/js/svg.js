...
(function () {
	...
	SVG.Color.test = function (color) {
		color += ''
		return SVG.regex.isHex.test(color) || SVG.regex.isRgb.test(color) || SVG.regex.isHsb.test(color)
	}
	...
	SVG.ViewBox = function (element) {
		var x, y, width, height, box = element.bbox(),
	...
	SVG.RBox = function (element) {
		var box, zoom, e = element.doc().parent,
	...
	SVG.extend(SVG.FX, {
		...
		animate: function (d, ease, delay) {
			...
			this.timeout = setTimeout(function () {
				...
				var akeys, tkeys, skeys, interval = 1000 / 60,
					element = fx.target,
					...
					fx.interval = setInterval(function () {
						...
						var i, key, time = new Date().getTime(),
						...
						if (fx._viewbox)
							element.viewbox(
								fx._at(fx._viewbox.x, pos), fx._at(fx._viewbox.y, pos), fx._at(fx._viewbox.width, pos), fx._at(fx._viewbox.height, pos))
